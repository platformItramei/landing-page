  ...
  const demoRequestMutation = useMutation({
    mutationFn: async (data: FullFormData) => {
      ...
      const auth = await apiRequest(
        'POST', 
        `${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/api/v1/user/auth/token`,
        {
      ...
      const backendPublicKey = await cryptos.importPublicKeyFromPem(
        `${process.env.NEXT_PUBLIC_BACKEND_PUBLIC_KEY}`
      );
      ...
      const response = await apiRequest(
        'POST',
        `${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/api/v1/user/auth/register`,
        epayload,
        token
      );
